...
- [ ] Task 2: Create Railway project (AC: 1)
  - [ ] 2.1 Create new Railway project
  - [ ] 2.2 Link GitHub repository for automatic deployments
  - [ ] 2.3 Provision PostgreSQL database add-on
  - [ ] 2.4 Configure environment variables (DATABASE_URL from Railway, NODE_ENV=production)

- [ ] Task 3: Configure Railway deployment settings (AC: 1, 4)
  - [ ] 3.1 Set build command: `npm run build`
  - [ ] 3.2 Set start command: `npm run start`
  - [ ] 3.3 Set root directory if needed
  - [ ] 3.4 Verify Node.js version compatibility

- [ ] Task 4: Push database schema (AC: 2)
  - [ ] 4.1 Run `npm run db:push` with Railway DATABASE_URL
  - [ ] 4.2 Verify tables are created in Railway PostgreSQL
  - [ ] 4.3 Test database connection from local with Railway URL

- [ ] Task 5: Deploy and verify (AC: 4, 5)
  - [ ] 5.1 Trigger deployment on Railway
